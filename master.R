...
datastory_name <- "gender_monitoring_confounders"
...
# characters etc.)
# -> Don't put "datastory" at the start or end!
datastory_name_en <- "the-impact-of-confounding-factors"
datastory_name_de <- "der-einfluss-von-stoerfaktoren"
datastory_name_fr <- "impact-des-variables-confondantes"

# English title and lead of the story (Mandatory, even if no EN version)
title_en <- "Gender monitoring (part 3): What influence do confounding factors have?" 
lead_en <- "Differences in success rates between women and men in SNSF funding can be explained by a variety of different direct and indirect influences. A look at confounding factors that can distort the results of analyses." 
# German title and lead of the story (Mandatory, even if no DE version)
title_de <- "Gender-Monitoring (Teil 3): Welchen Einfluss haben Störfaktoren?" 
lead_de <- "Unterschiede zwischen den Erfolgsquoten von Frauen und Männern in der SNF-Förderung lassen sich durch vielfältige direkte und indirekte Einflüsse erklären. Ein Blick auf Störfaktoren, die das Ergebnis von Analysen verzerren können." 
# French title and lead of the story (Mandatory, even if no FR version)
title_fr <- "Gender monitoring (3) : Quelle est l'influence des facteurs confondants ?"
lead_fr <- "Les disparités observées dans les subsides alloués par le FNS selon le genre des personnes requérantes peuvent s’expliquer par diverses influences directes et indirectes : coup d'œil sur les facteurs confondants qui peuvent fausser les résultats des analyses." 
# Contact persons, always (first name + last name)
contact_person <- c("Oliver John", "Manuela Ott")
...
publication_date <- "2022-11-09 01:00:00"
...
doi_url <- "https://doi.org/10.46446/datastory.gender-monitoring-confounders"
# URL to Github page (optional)
github_url <- "https://github.com/snsf-data/datastory_gender_monitoring_confounders" 
